...
async function loadLanguages1() {

     let i = 0;
     let menuLanguages = document.getElementById("id-languages1");
     let ii = languages.findIndex(rec => rec.lid === activeLanguageID);

     removeElements('id-languages1');

     let div = document.createElement("div");
     div.id = 'id-languageHeader1';
     div.classList.add('cs-languageHeader');
...
async function loadVersions1() {
     ...
     div1 = document.createElement("div");
     div1.id = 'id-versionHeaderlanguage1';
     div1.classList.add("cs-versionHeaderLanguage");
     div1.textContent = `Language: ${languages[ii].lng}`;
     div.appendChild(div1);
     menuVersions.appendChild(div);

     while (i < versions.length && versions[i].lid === Number(activeLanguage1ID)) {
          ...
          div.id = `id-versionA${versions[i].id}`;
          if (activeVersion1ID === div.id) {
               menuVersion.textContent = versions[i].ar;
               pageHeadline.textContent = versions[i].t;
          };
          div.dataset.index = i;
