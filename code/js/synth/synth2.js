...
function listVoices1() {

     let avoice;
     voices = speechSynthesis.getVoices();
     if (voices) {
          voices.forzEach((voice, index) => {
               if (voice) {
                    avoice = {
                         idx: index,
                         name: voice.name,
...
function loadSpeechText() {
     ...
     return new Promise(resolve => {
          ...
          textSpeech = '';
          if (versions[idx].ar === 'DRB') {
               textSpeech = `Doooey Rheems Bible: ${book}: Chapter ${activeChapter}: `;
          } else { textSpeech = `${versions[idx].t}: ${book}: Chapter ${activeChapter}: `; };
          const elements = document.querySelectorAll('[id*="id-avers"]');
          for (const el of elements) {
               let x = Number(el.id.slice("id-avers".length));
...
async function waitForTranslation2() {
     const target = document.getElementById('id-page');
     if (!target) return;

     // Count verses or text nodes to estimate page size
     //const verseCount = target.querySelectorAll('.verse').length || 20;
     const verseCount = target.querySelectorAll('[id*="id-avers"]').length || 20;

     // Dynamic debounce: small pages translate fast, big pages need more time
     const debounce = Math.min(1200, Math.max(300, verseCount * 5));

     // Max wait time: prevents infinite waiting on huge pages
     const maxWait = Math.max(2000, verseCount * 15);

     return new Promise(resolve => {
          let timeout;
          let lastMutation = Date.now();
          let maxTimer;
          console.log(`start: ${lastMutation}`);

          const observer = new MutationObserver(() => {
               lastMutation = Date.now();
               clearTimeout(timeout);
               timeout = setTimeout(checkIfDone, debounce);
          });
          ...
          // Safety cap: if translation takes too long, resolve anyway
          maxTimer = setTimeout(() => {
               observer.disconnect();
               console.log(`max: ${Date.now}`);
               resolve(true);
          }, maxWait);
...
async function waitForTranslation1() {
     const target = document.getElementById('id-page');
     if (!target) return;
...
async function waitForTranslation() {
     const verses = [...document.querySelectorAll('[id*="id-avers"]')];
     ...
     return new Promise(resolve => {
          ...
          const debounce = 1200;  // long enough for big chapters
          const maxWait = 15000; // safety cap

          const observer = new MutationObserver(mutations => {
               let relevant = false;

               for (const m of mutations) {
                    if (verses.some(v => v.contains(m.target) || v === m.target)) {
                         relevant = true;
                         break;
                    }
               }
          ...
          // Observe the entire document, but filter mutations manually
...
async function nextSynthChapter() {
     ...
     setTimeout(async () => {
          ...
          let rec = await getChapter();
          if (rec) { rec = false; rec = await loadChapters(changeChapter); };
          if (rec) { rec = false; rec = await waitForTranslation(); };
          //if (rec) { rec = false; rec = await loadSpeechText(); };
